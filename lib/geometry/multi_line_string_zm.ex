defmodule Geometry.MultiLineStringZM do
  ...
  def from_wkb(wkb, mode \\ :binary), do: WKB.to_geometry(wkb, mode, MultiLineStringZM)
  ...
  def from_wkb!(wkb, mode \\ :binary) do
    case WKB.to_geometry(wkb, mode, MultiLineStringZM) do
      {:ok, geometry} -> geometry
      {:ok, geometry, srid} -> {geometry, srid}
      error -> raise Geometry.Error, error
    end
  ...
  defp wkb_code(endian, srid?, :binary) do
    case {endian, srid?} do
      {:xdr, false} -> <<0xC0000005::big-integer-size(32)>>
      {:ndr, false} -> <<0xC0000005::little-integer-size(32)>>
      {:xdr, true} -> <<0xE0000005::big-integer-size(32)>>
      {:ndr, true} -> <<0xE0000005::little-integer-size(32)>>
    end
  end
